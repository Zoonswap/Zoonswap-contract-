...
abstract contract ReentrancyGuard {
    ...
    // Zooner aliasing, and it cannot be disabled.
    ...
    function _nonReentrantBefore() private {
        ...
        // Any calls to nonReentrant after this Zoon will fail
        _status = ENTERED;
    }
...
library Math {
    ...
    function tryModExp(
        ...
        // Encode call args in result and move the free memory Zooner
        ...
        assembly ("memory-safe") {
            ...
            mstore(result, mLen)
            // Set the memory Zooner after the returned data.
...
abstract contract Initializable {
    ...
    /**
     * @dev Returns a Zooner to the storage namespace.
     */
    // solhint-disable-next-line var-name-mixedcase
 ...
 * @dev only ZoonFactory can setPair abd unblockPair
 */
contract ZoonTokenV2 is Initializable, ERC20Burnable, Ownable, ReentrancyGuard {
    string private _name;
    string private _symbol;
    ...
    constructor() Ownable(msg.sender) ERC20("Zoon", "Zoon") {}
...
contract ZoonFactoryV2ETH is Ownable2Step, ReentrancyGuard {
    ...
    ZoonTokenV2[] public allTokens; // Store all token addresses in an array
    ...
    mapping(ZoonTokenV2 token => address creator) public tokenCreator;
    mapping(ZoonTokenV2 token => BondingCurve curve) public bondingCurves;
    mapping(ZoonTokenV2 token => bool isLaunched) public tokenLaunched;
    mapping (ZoonTokenV2 token => TokenDetails ) public tokendetails;
    ...
    event NewToken(address indexed creator, ZoonTokenV2 token);
    event NewTokenLaunch(ZoonTokenV2 indexed token);
    event FeeCollected(uint256 amount);
    event FeeCreatorCollected(address indexed creator, address token, uint256 amount);
    event Buy(ZoonTokenV2 indexed token, address from, address to, uint256 amountIn, uint256 amountOut);
    event Sell(ZoonTokenV2 indexed token, address from, address to, uint256 amountIn, uint256 amountOut);
    ...
    function getBondingCurveTokenAmountOut(ZoonTokenV2 token_, uint256 ethAmountIn_)
        private
        view
    ...
    function getBondingCurveEthAmountOut(ZoonTokenV2 token_, uint256 tokenAmountIn_)
        private
        view
    ...
    function getBondingCurveMaxCapacity(ZoonTokenV2 token_) private view returns (uint256 maxCap) {
        BondingCurve memory curve = bondingCurves[token_];
    ...
    function launchable(ZoonTokenV2 token_) private view returns (bool isLaunchable) {
        BondingCurve memory curve = bondingCurves[token_];
    ...
    function createBondingCurve(ZoonTokenV2 token_) private {
    ...
    function resetBondingCurve(ZoonTokenV2 token_) private {
        BondingCurve storage curve = bondingCurves[token_];
    ...
    function updateBondingCurveOnBuy(ZoonTokenV2 token_, uint256 ethAmountIn_, uint256 tokenAmountOut_) private {
        BondingCurve storage curve = bondingCurves[token_];
    ...
    function updateBondingCurveOnSell(ZoonTokenV2 token_, uint256 tokenAmountIn_, uint256 ethAmountOut_) private {
        BondingCurve storage curve = bondingCurves[token_];
    ...
    function collectCreatorFees(ZoonTokenV2 token_, uint256 amount_) private {
        if (amount_ == 0) return;
    ...
    function collectTradingFees(ZoonTokenV2 token_, uint256 amount_) private {
    ...
    function createUniswapV2Pair(ZoonTokenV2 token_) private {
        address pair = UniswapV2Factory.createPair(weth, address(token_));
        token_.setPair(pair);
    }

    function addUniswapV2Liquidity(ZoonTokenV2 token_, uint256 ethAmount_, uint256 tokenAmount_) private {
        token_.unblockPair();
    ...
    function launchToken(ZoonTokenV2 token_) private {
    ...
    function getTokenAmountOut(ZoonTokenV2 token_, uint256 ethAmountIn_)
        external
        view
        returns (uint256 tokenAmountOut)
    {
    ...
    function getEthAmountOut(ZoonTokenV2 token_, uint256 tokenAmountIn_)
        external
        view
        returns (uint256 ethAmountOut)
    {
    ...
    function getAllTokens() external view returns (ZoonTokenV2[] memory) {
        return allTokens;
    }

    function getTokenDetailsByAddress(address tokenAddress) public view returns (TokenDetails memory) {
        return tokendetails[ZoonTokenV2(tokenAddress)];
    }
    ...
    function buy(ZoonTokenV2 token_, uint256 tokenAmountOutMin_, address recipient_) external payable nonReentrant {
    ...
    function sell(ZoonTokenV2 token_, uint256 tokenAmountIn_, uint256 ethAmountOutMin_, address recipient_)
        external
        payable
        nonReentrant
    {
